...
# v1.0.0dm: Scheduled agents -----------------------------------------------
# Recurring jobs that run a persona on a cadence (cron / Railway scheduled
# job hits the run endpoint, or run_scheduled.py runs them directly). The
# latest run of each job is persisted so the UI can show "here's last
# Monday's digest" and trigger a job on demand.

@app.route("/api/agent/scheduled", methods=["GET"])
def api_agent_scheduled_list():
    """List the scheduled agent jobs plus each one's latest run."""
    import scheduled_agents
    return jsonify({"jobs": scheduled_agents.list_jobs()})
...
def api_agent_scheduled_run(key):
    ...
    import scheduled_agents
    record = scheduled_agents.run_job(key, actor=_actor())
    if record.get("error"):
        status = 404 if record.get("code") == "unknown_job" else 400
...
def api_watchlist_sweep():
    ...
    # v1.0.0dm: the sweep body now lives in watchlist_sweep.run_sweep so
    # scheduled_agents can run it without HTTP. This endpoint is a thin
    # wrapper that adds the audit event.
    import watchlist_sweep
    summary = watchlist_sweep.run_sweep(only=only)
